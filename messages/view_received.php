...
// Suppression automatique si autodestruction
if ($message['self_destruct']) {
    $delete = $pdo->prepare("DELETE FROM private_messages WHERE id = ?");
    $delete->execute([$id]);
}
?>
...
(function(){
    ...
    // Si on quitte la page OU que le contenu de #app est vidé (via transition)
